module Ingestions
  module Configuration
    ...
    class AbstractRegistry
      ...
      moddable_klass! :configurator, Ingestions::Configuration::AbstractRegistryConfigurator
      moddable_klass! :definition, Ingestions::Configuration::AbstractDefinition
      moddable_klass! :definition_configurator, Ingestions::Configuration::AbstractDefinitionConfigurator
      ...
      def [](name)
        find_by_name(name) or raise Ingestions::Configuration::UnknownDefinition, "Unknown definition: #{name}"
      end
      ...
      def add(position: nil, **options)
        ...
        raise Ingestions::Configuration::InvalidDefinition, new_definition.errors.full_messages.to_sentence unless new_definition.valid?

        raise Ingestions::Configuration::AlreadyDefined, "Already defined: #{new_definition.name}" unless @definitions.add?(new_definition)
      ...
      class << self
        ...
        def infer_defaults!(definition_definer: infer_definition_definer, interaction_namespace: infer_interaction_namespace, interaction_parent: "#{interaction_namespace}::Abstract#{infer_noun_klass}".safe_constantize)
          registry_klass = self

          if definition_definer.present?
            configurator do
              define_method(definition_definer) do |name, interaction = nil, &block|
                configurator = registry_klass.definition_configurator.new(name, interaction, &block)
